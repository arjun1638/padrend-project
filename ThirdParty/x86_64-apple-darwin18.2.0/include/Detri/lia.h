...
#endif
/* This macro returns the number of Lia digits necessary to represent upto
   D decimal digits.  NOTE: This is a slightly conservative estimate; eg, for
   32-bit architectures, Lia digits hold upto 2^30 == 1073741824 (see base.h);
   still, we count it only as 9 decimal digits. This could be optimized using
   binary representation. */

#define LIA_DOUBLE_MAXDECIMALS 16
/* 64-bit (double) IEEE 745 floating-point arithmetic corresponds to roughly
   16 significant decimal digits.  IMPORTANT: Check with aux.c, if you decide
